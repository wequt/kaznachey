        ...
        <div class="max-w-7xl mx-auto space-y-6 sm:space-y-8">
            ...
            <div class="flex justify-between items-center gap-4">
                <div class="min-w-0">
                    ...
                    <p class="text-sm sm:text-base text-slate-500 italic mt-0.5 sm:mt-0 truncate">Аналитическая панель
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
                ...
                <div class="lg:col-span-8 space-y-6 sm:space-y-8">
                    ...
                    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                        <div
                            class="p-5 sm:p-8 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-linear-to-r from-white to-slate-50/50">
                            <div>
                            ...
                            <div class="text-left sm:text-right">
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">
                                    Доступно средств</p>
                                <p class="text-xl sm:text-2xl font-black text-[#A66353]">{{ formatMoney(totalBalance) }}
                                </p>
                            </div>
                        </div>
                            ...
                            <div v-for="tx in recentTransactions" :key="tx.id"
                                ...
                                <div class="flex items-center gap-3 sm:gap-5 min-w-0">
                                    <div :class="tx.destination_account_id ? 'bg-amber-50 text-amber-600' : (tx.category?.type === 'expense' ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500')"
                                        ...
                                        <ArrowLeftRight v-if="tx.destination_account_id"
                                            class="w-5 sm:w-6 h-5 sm:h-6" />
                                        <ArrowDownLeft v-else-if="tx.category?.type === 'expense'"
                                            class="w-5 sm:w-6 h-5 sm:h-6" />
                                        <ArrowUpRight v-else class="w-5 sm:w-6 h-5 sm:h-6" />
                                    </div>
                                    <div class="min-w-0">
                                        <p class="font-bold text-slate-800 text-sm sm:text-base truncate">
                                            {{ tx.destination_account_id ? 'Перевод между счетами' : (tx.description ||
                                            tx.category?.name) }}
                                        </p>
                                        <div
                                            class="flex items-center gap-2 text-[11px] sm:text-xs text-slate-400 mt-0.5 flex-wrap">
                                            <template v-if="tx.destination_account_id">
                                                <span
                                                    class="bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 font-medium text-ellipsis overflow-hidden">{{
                                                    tx.account?.name }}</span>
                                                <span>→</span>
                                                <span
                                                    class="bg-amber-50 px-1.5 py-0.5 rounded text-amber-800 font-medium text-ellipsis overflow-hidden">{{
                                                    tx.destination_account?.name }}</span>
                                            </template>
                                            <template v-else>
                                                <span
                                                    class="bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 font-medium text-ellipsis overflow-hidden">{{
                                ...
                                <div class="text-right shrink-0">
                                    <p class="text-base sm:text-lg font-black"
                                        ...
                                        {{ tx.destination_account_id ? '' : (tx.category?.type === 'expense' ? '-' :
                                        '+') }}{{ formatMoney(tx.amount) }}
                                    </p>
                                    <p v-if="tx.destination_account_id || tx.category?.name"
                                        class="text-[9px] sm:text-[10px] uppercase font-bold text-slate-400 mt-0.5 truncate max-w-25 sm:max-w-none">
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="bg-white p-5 sm:p-8 rounded-3xl border border-slate-100 shadow-sm">
                        ...
                        <div class="relative h-48 sm:h-56 mb-6 sm:mb-8">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                <span
                                    class="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Траты
                                    (мес.)</span>
                                <span class="text-lg sm:text-xl font-black text-[#A66353]">{{ formatMoney(totalSpent)
                                    }}</span>
                            </div>
                        </div>
                    ...
                    <div
                        class="bg-[#A66353] p-6 sm:p-8 rounded-3xl text-white shadow-xl shadow-[#a663532c] relative overflow-hidden group">
                        ...
                        <div
                            class="absolute -right-10 -bottom-10 w-40 h-40 bg-white opacity-5 rounded-full group-hover:scale-110 transition-transform duration-700">
                        </div>
                    </div>
                </div>
